-- 1. Check Tina Test's leave requests
SELECT 
  id,
  user_id,
  start_date,
...
-- 2. Check if Tina Test is in any teams
SELECT 
  tm.id,
  tm.user_id,
  tm.team_id,
...
-- 3. Check if Anna Admin is a TEAMLEAD in Tina's teams
SELECT 
  tm.id,
  tm.user_id,
  tm.team_id,
...
JOIN users u ON tm.user_id = u.id
WHERE tm.team_id IN (
  -- Teams where Tina Test is a member
...
-- 4. Check Anna Admin's role and team memberships
SELECT 
  u.id,
  u.email,
  u.first_name,
...
-- 5. Check if withdrawn_at and cancelled_at columns exist
SELECT 
  column_name,
  data_type,
  is_nullable
...
-- FIX 1: If Tina Test is not in any team, add her to a team
-- First, find or create a team
DO $$
DECLARE
  v_team_id uuid;
  v_tina_id uuid;
...
BEGIN
  ...
  -- Check if Tina is already in a team
  ...
  IF v_team_id IS NULL THEN
    ...
    -- If no team was created, get an existing one
    IF v_team_id IS NULL THEN
      SELECT id INTO v_team_id FROM teams LIMIT 1;
    END IF;
    
    -- Add Tina to the team as MEMBER
  ...
  -- Check if Anna is a TEAMLEAD in that team
  IF NOT EXISTS (
    ...
    -- Add Anna as TEAMLEAD or update her role
...
-- FIX 2: Ensure withdrawn_at and cancelled_at columns exist
DO $$
BEGIN
  -- Add withdrawn_at if it doesn't exist
  IF NOT EXISTS (
  ...
  -- Add cancellation_confirmed if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
...
-- Show all leave requests for Tina (including withdrawn/cancelled)
SELECT 
