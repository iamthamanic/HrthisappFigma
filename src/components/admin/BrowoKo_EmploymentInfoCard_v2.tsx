/**
 * @file BrowoKo_EmploymentInfoCard.tsx (v4.9.3 - Wochenstunden mit Gültig-ab Datum)
 * @domain HR - Team Member Details
 * @description Employment information form card with card-level editing
 * Includes: Basic employment info, break settings, work time model, on-call status, and weekly hours history
 * @updated v4.9.3 - Added "Gültig ab" date for weekly hours changes with contract reminder
 */
...
import { Briefcase, Edit, X, Save, AlertTriangle, FileText } from '../icons/BrowoKoIcons';
...
export function EmploymentInfoCard({
  ...
  // v4.9.3: Weekly hours change tracking
  const [weeklyHoursChanged, setWeeklyHoursChanged] = useState(false);
  const [weeklyHoursValidFrom, setWeeklyHoursValidFrom] = useState<string>('');
  ...
  const handleEdit = () => {
    ...
    setWeeklyHoursChanged(false);
    setWeeklyHoursValidFrom('');
  ...
  // v4.9.3: Handle weekly hours change
  const handleWeeklyHoursChange = (value: number) => {
    const newValue = value || 0;
    setFormData({ ...formData, weekly_hours: newValue });
    
    // Check if value changed
    if (newValue !== (user?.weekly_hours || 40)) {
      setWeeklyHoursChanged(true);
      // Default to today if not set
      if (!weeklyHoursValidFrom) {
        setWeeklyHoursValidFrom(new Date().toISOString().split('T')[0]);
      }
    } else {
      setWeeklyHoursChanged(false);
      setWeeklyHoursValidFrom('');
  ...
  const handleSave = async () => {
    // v4.9.3: Validate "Gültig ab" date if weekly hours changed
    if (weeklyHoursChanged) {
      if (!weeklyHoursValidFrom) {
        toast.error('Bitte geben Sie das "Gültig ab" Datum an');
        return;
      }
      
      // Validate: Datum darf nicht vor Eintrittsdatum liegen
      if (formData.start_date && weeklyHoursValidFrom < formData.start_date) {
        toast.error('Das "Gültig ab" Datum darf nicht vor dem Eintrittsdatum liegen');
        return;
      }
    }
    ...
    setSaving(true);
    try {
      // v4.9.3: Handle weekly hours change BEFORE updating users table
      if (weeklyHoursChanged && weeklyHoursValidFrom) {
        // 1. Close old weekly_hours_history entries
        const yesterdayDate = new Date(weeklyHoursValidFrom);
        yesterdayDate.setDate(yesterdayDate.getDate() - 1);
        const validTo = yesterdayDate.toISOString().split('T')[0];
        ...
        if (closeError) {
          console.error('Error closing old weekly hours history:', closeError);
          // Continue anyway - this is not critical
        }
        
        // 2. Create new weekly_hours_history entry
        const { error: historyError } = await supabase
          .from('weekly_hours_history')
          .insert({
            user_id: user.id,
            weekly_hours: formData.weekly_hours,
            valid_from: weeklyHoursValidFrom,
            valid_to: null,
            change_reason: `Vertragsänderung: ${user.weekly_hours}h → ${formData.weekly_hours}h`,
            changed_by: currentUser?.id || null
          });
        
        if (historyError) {
          console.error('Error creating weekly hours history:', historyError);
          throw new Error('Fehler beim Erstellen der Wochenstunden-Historie: ' + historyError.message);
        }
      }
      ...
      // v4.9.3: Success message mit Hinweis auf Wochenstunden-Änderung
      if (weeklyHoursChanged) {
        toast.success(
          `Firmeninformationen gespeichert. Wochenstunden ab ${new Date(weeklyHoursValidFrom).toLocaleDateString('de-DE')} geändert.`,
          { duration: 5000 }
        );
        
        if (showContractReminder) {
          toast.info(
            'Hinweis: Bitte neuen Arbeitsvertrag mit geänderten Wochenstunden erstellen und hochladen.',
            { duration: 7000 }
          );
        }
      } else {
        ...
        {isEditing ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {/* v4.9.3: Wochenstunden mit "Gültig ab" Datum */}
              <div className="space-y-2 md:col-span-2">
                ...
                {/* Warning wenn Wochenstunden geändert wurden */}
                {weeklyHoursChanged && (
                      ...
                      <div className="space-y-3">
                        <div>
                          <p className="font-semibold text-orange-900 mb-1">
                            ⚠️ Wochenstunden-Änderung: {user.weekly_hours}h → {formData.weekly_hours}h
                          </p>
                          <p className="text-orange-800">
                            Diese Änderung erfordert eine Vertragsanpassung.
                          </p>
                        </div>
                        
                        {/* Gültig ab Datum */}
                        <div className="space-y-2">
                          <Label htmlFor="weekly_hours_valid_from" className="text-orange-900">
                            Gültig ab: *
                          </Label>
                          <Input
                            id="weekly_hours_valid_from"
                            type="date"
                            value={weeklyHoursValidFrom}
                            onChange={(e) => setWeeklyHoursValidFrom(e.target.value)}
                            min={formData.start_date || undefined}
                            className="bg-white border-orange-300 focus:border-orange-500"
                            required
                          />
                          <p className="text-xs text-orange-700">
                            Ab diesem Datum gelten die neuen Wochenstunden ({formData.weekly_hours}h).
                            Zeitkonten vor diesem Datum bleiben mit {user.weekly_hours}h berechnet.
                          </p>
                        </div>
                        
                        {/* Vertrags-Reminder Checkbox */}
                        <div className="flex items-start gap-2 p-3 bg-white rounded border border-orange-200">
                          <input
                            type="checkbox"
                            id="contract_reminder"
                            checked={showContractReminder}
                            onChange={(e) => setShowContractReminder(e.target.checked)}
                            className="mt-1 w-4 h-4 text-orange-600 border-gray-300 rounded focus:ring-orange-500"
                          />
                          <label htmlFor="contract_reminder" className="text-sm text-gray-700 cursor-pointer">
                            <div className="flex items-center gap-1.5 font-medium text-gray-900 mb-1">
                              <FileText className="w-4 h-4" />
                              Neuen Arbeitsvertrag erstellen
                            </div>
                            <p className="text-xs text-gray-600">
                              Ich werde nach dem Speichern einen neuen Arbeitsvertrag mit {formData.weekly_hours}h/Woche 
                              erstellen und im Tab "Dokumente" hochladen.
                            </p>
                          </label>
                        </div>
                      </div>
            ...
            {/* Break Settings Section - Truncated for brevity, same as v1 */}
            {/* ... Rest of the form identical to v1 ... */}
