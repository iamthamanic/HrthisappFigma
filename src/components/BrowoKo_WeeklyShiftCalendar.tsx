...
export function BrowoKo_WeeklyShiftCalendar({
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="w-full overflow-x-auto overflow-y-auto border border-gray-200 rounded-md max-h-[600px]">
        <div className="min-w-[1800px]">
          {/* Timeline Header: Hours */}
          <div className="grid grid-cols-[140px_1fr] gap-0 border-b border-gray-200 bg-gray-50 sticky top-0 z-10">
            <div className="px-4 py-2 border-r border-gray-200 font-medium text-sm bg-gray-50 sticky left-0 z-20">Tag</div>
            <div className="relative flex">
              {HOUR_SLOTS.filter((_, i) => i % 2 === 0).map((time, index) => (
                <div
                  key={time}
                  className="flex-1 text-center text-xs text-gray-600 py-2 border-r border-gray-200 min-w-[60px]"
                >
                  {time}
                </div>
              ))}
          ...
          <div>
            {WEEKDAYS.map((weekday, dayIndex) => {
              ...
              return (
                <div
                  key={dayIndex}
                  className={`grid grid-cols-[140px_1fr] gap-0 border-b border-gray-200 min-h-[80px] ${
                    isToday ? 'bg-blue-50' : isPast ? 'bg-gray-50' : 'bg-white'
                  }`}
                >
                  {/* Day Label */}
                  <div className={`px-4 py-3 border-r border-gray-200 flex flex-col justify-start sticky left-0 z-20 ${
                    isToday ? 'bg-blue-50' : isPast ? 'bg-gray-50' : 'bg-white'
                  }`}>
                    ...
                    <div className="relative z-10 flex flex-col gap-1 px-2">
                      {dayShifts.map((shift) => {
                        ...
                        return (
                          ...
                          >
                            <div
                              className="flex items-center gap-2 px-2 py-1.5 rounded shadow-sm hover:shadow-md transition-shadow border border-gray-300"
