-- ============================================================================
-- v4.13.1 - TEST BUILDER FOUNDATION - DATABASE MIGRATION (SAFE VERSION)
-- ============================================================================
...
-- RLS Policies (SAFE: Drop first, then create)
ALTER TABLE public.tests ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view tests in their organization" ON public.tests;
...
DROP POLICY IF EXISTS "HR and Superadmin can create tests" ON public.tests;
...
DROP POLICY IF EXISTS "HR and Superadmin can update tests" ON public.tests;
...
DROP POLICY IF EXISTS "HR and Superadmin can delete tests" ON public.tests;
...
DROP POLICY IF EXISTS "Users can view test blocks via test access" ON public.test_blocks;
...
DROP POLICY IF EXISTS "HR and Superadmin can create test blocks" ON public.test_blocks;
...
DROP POLICY IF EXISTS "HR and Superadmin can update test blocks" ON public.test_blocks;
...
DROP POLICY IF EXISTS "HR and Superadmin can delete test blocks" ON public.test_blocks;
...
-- Ensure 1:1 relationship (one test per video) - only if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'test_video_assignments_video_id_key'
  ) THEN
    ALTER TABLE public.test_video_assignments 
    ADD CONSTRAINT test_video_assignments_video_id_key UNIQUE(video_id);
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Users can view test assignments in their organization" ON public.test_video_assignments;
...
DROP POLICY IF EXISTS "HR and Superadmin can create test assignments" ON public.test_video_assignments;
...
DROP POLICY IF EXISTS "HR and Superadmin can delete test assignments" ON public.test_video_assignments;
...
DROP TRIGGER IF EXISTS tests_updated_at_trigger ON public.tests;
...
DROP TRIGGER IF EXISTS test_blocks_updated_at_trigger ON public.test_blocks;
...
BEGIN
  ...
  RAISE NOTICE '1. App neu laden (STRG+R / CMD+R)';
  RAISE NOTICE '2. Gehe zu: Admin → Lernverwaltung → Tests';
  RAISE NOTICE '3. Erstelle deinen ersten Test! 🚀';
END $$;
