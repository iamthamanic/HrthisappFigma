-- ================================================
-- COMPLETE MISSING TABLES SETUP
-- ================================================
-- This migration creates all missing tables that might not have been created yet
-- Run this AFTER you've run migrations 001-011
-- Run this BEFORE migration 016 (Multi-Tenancy)

-- ================================================
-- 1. ACTIVITY FEED (from migration 012)
-- ================================================
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Anyone can read activity_feed'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'activity_feed' 
    AND policyname = 'System can insert activity_feed'
  ) THEN
...
DROP TRIGGER IF EXISTS trigger_video_completed ON learning_progress;
...
DROP TRIGGER IF EXISTS trigger_level_up ON user_avatars;
...
DROP TRIGGER IF EXISTS trigger_coins_earned ON coin_transactions;
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'activity_feed') THEN
    RAISE NOTICE '✅ activity_feed table created successfully';
  ELSE
    RAISE NOTICE '❌ WARNING: activity_feed table was not created!';
  END IF;
END $$;
